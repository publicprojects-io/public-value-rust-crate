...
| `leakage_adjustment`, `multiplier_effect` | `foundations` | `additionality-and-deadweight`'s own "The maths" section states the full net-impact sequence as "gross − deadweight − displacement − leakage, × multiplier" but this crate's `net_additional_outcomes` (deadweight) and `net_additional_impact` (displacement) don't implement the leakage or multiplier steps the source doc names. | UK Additionality Guide (BIS, 4th edition, 2014): `gross × (1 − substitution) × (1 − leakage) × (1 − displacement) × multiplier`; IMPLAN economic-impact multiplier methodology for the worked example. |
| `real_value` | `economic_appraisal` | `social-discount-rate`'s pitfalls section warns against "discounting real and nominal cash flows inconsistently" but provides no real/nominal conversion itself. | Standard GDP-deflator method: `Real = Nominal / Deflator × 100`. |
| `value_of_statistical_life` | `economic_appraisal` | `social-cost-benefit-analysis` cites HM Treasury's published VSL figure (≈£2.1m) as a given input without deriving it; `revealed-preference-valuation` implements the same hedonic logic for house prices but not wages. | Revealed-preference wage-risk (hedonic wage) method; compensating wage differential literature (Viscusi, NBER). |
| `internal_rate_of_return` | `economic_appraisal` | The standard companion figure to net present value in a `FiveCaseModel`/`social-cost-benefit-analysis` economic case; Green Book appraisals routinely report both, but this crate only had NPV. | Standard IRR definition: the discount rate at which a cash-flow series' NPV is zero, found here by bisection. |
