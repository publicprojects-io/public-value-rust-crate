...
pub fn cost_of_delay_per_week(annual_benefit: Money) -> Money {
    annual_benefit / Decimal::from(52)
}
...
/// assert_eq!(loss.value().round_dp(0), dec!(3_000_000));
/// ```
#[must_use]
...
pub fn technical_debt_principal(lines_of_code: u32, remediation_cost_per_line: Money) -> Money {
    remediation_cost_per_line * lines_of_code
}
...
/// use rust_decimal_macros::dec;
///
/// let tdr = technical_debt_ratio(Money::new(dec!(712_500)), Money::new(dec!(4_453_125)));
...
pub fn technical_debt_ratio(remediation_cost: Money, redevelopment_cost: Money) -> Percentage {
    Percentage::from_fraction(remediation_cost.ratio_to(redevelopment_cost).value())
}
...
/// let reduction = interest_reduction(Money::new(dec!(680_000)), Percentage::from_percent(70.0));
...
pub fn interest_reduction(current_annual_interest: Money, reduction_fraction: Percentage) -> Money {
    ...
    current_annual_interest * fraction
}
...
pub fn payback_period_years(remediation_cost: Money, annual_interest_reduction: Money) -> f64 {
    remediation_cost.ratio_to(annual_interest_reduction).value()
}
...
pub fn total_cost_of_ownership(acquisition_cost: Money, annual_operating_cost: Money, annual_rate: Decimal, years: u32) -> Money {
    acquisition_cost + annual_operating_cost * annuity_factor(annual_rate, years)
}
...
/// use rust_decimal_macros::dec;
///
/// let adjusted = risk_adjusted_build_cost(Money::new(dec!(900_000)), dec!(1.4));
/// assert_eq!(adjusted.value(), dec!(1_260_000.0));
/// ```
#[must_use]
pub fn risk_adjusted_build_cost(build_cost_estimate: Money, optimism_bias_multiplier: Decimal) -> Money {
    build_cost_estimate * optimism_bias_multiplier
}
...
/// use rust_decimal_macros::dec;
///
/// let cost = buy_total_cost(Money::new(dec!(180_000)), 5);
/// assert_eq!(cost.value(), dec!(900_000));
/// ```
#[must_use]
pub fn buy_total_cost(annual_subscription: Money, years: u32) -> Money {
    annual_subscription * years
}
...
/// use rust_decimal_macros::dec;
///
/// let value = ai_capacity_value(210, 0.2, 220, Money::new(dec!(55)), Percentage::from_percent(60.0));
...
pub fn ai_capacity_value(eligible_developers: u32, hours_saved_per_day: f64, working_days_per_year: u32, loaded_hourly_rate: Money, utilization: Percentage) -> Money {
    ...
    loaded_hourly_rate * hours_decimal
}
...
/// use rust_decimal_macros::dec;
///
/// let ratio = net_capacity_ratio(Money::new(dec!(304_920)), Money::new(dec!(55_440)));
/// assert!((ratio.value() - 5.5).abs() < 0.1);
/// ```
#[must_use]
pub fn net_capacity_ratio(capacity_value: Money, licensing_cost: Money) -> Ratio {
    capacity_value.ratio_to(licensing_cost)
}
...
/// let value = lead_time_value_pulled_forward(25, weeks_saved, Money::new(dec!(8_000)));
/// assert!((value.value() - dec!(1_457_143)).abs() < dec!(1_000));
/// ```
#[must_use]
pub fn lead_time_value_pulled_forward(improvements_per_year: u32, lead_time_reduction_weeks: f64, weekly_cost_of_delay: Money) -> Money {
    ...
    weekly_cost_of_delay * weeks * improvements_per_year
}
...
///
/// let fewer = fewer_failed_changes(25, Percentage::from_percent(30.0), Percentage::from_percent(10.0));
/// let saving = incident_avoidance_saving(fewer, Money::new(dec!(16_840)));
